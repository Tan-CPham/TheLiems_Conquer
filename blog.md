...
Với **[Project Warmup Đợt 1: Dự báo rủi ro nghỉ việc của nhân sự](https://aioconquer.aivietnam.edu.vn/posts/project-warmup-dot-1-du-bao-rui-ro-nghi-viec-cua-nhan-su)** — nhóm đã xây dựng mô hình **Random Forest và Logistic Regression** để dự báo attrition — chúng em nhận ra rằng việc sử dụng dataset IBM chưa được xử lý kỹ đã khiến hiệu suất mô hình bị giới hạn đáng kể (**Trash in - Trash out**).
...
#### a. Kiểm tra data bị thiếu và data không hợp lệ
...
![Bảng IQR Summary](ảnh_của_bạn)
*Hình X: Bảng thống kê outlier theo phương pháp IQR*

![Boxplot các biến có outlier](ảnh_của_bạn)
*Hình X: Boxplot phân bố các biến số có outlier*
...
#### c. Kiểm tra phân bố cột Label (Attrition)
...
![Phân bố Attrition](ảnh_attrition_của_bạn)
*Hình 3: Phân bố tỷ lệ Attrition — Yes vs No*
...
#### d. Phân bố các đặc trưng (Feature Distribution)

**Numeric Features**

![Phân bố các biến số](ảnh_numeric_distribution_của_bạn)
*Hình 4: Histogram phân bố các đặc trưng dạng số*
...
![Phân bố Attrition theo Categorical Features](ảnh_categorical_distribution_của_bạn)
*Hình 5: Tỷ lệ Attrition theo từng đặc trưng phân loại*
...
#### a. Loại bỏ các cột không có giá trị
...
#### b. Loại bỏ các cột có tương quan cao

![h](ảnh_của_bạn)
*Hình X: Ma trận tương quan (Correlation Matrix) của các biến số*
...
![Heatmap tương quan](ảnh_heatmap_của_bạn)
*Hình X: Heatmap tương quan giữa các đặc trưng số*

#### c. Tạo các đặc trưng mới
...
#### a. SMOTE-NC (Synthetic Minority Oversampling Technique for Nominal and Continuous)
...
**B1:** Xác định lớp thiểu số: Chọn một mẫu dữ liệu $x_i$ từ tập hợp các mẫu thuộc lớp thiểu số.  
...
**Bước 3 — Chọn một láng giềng:** Chọn ngẫu nhiên một trong k láng giềng đó, gọi là **x_zi**.

**Bước 4 — Tạo mẫu tổng hợp:** Mẫu mới **x_new** được tạo ra theo công thức:

| Thành phần | Ý nghĩa |
|---|---|
| x_new = x_i + λ × (x_zi - x_i) | Công thức nội suy |
| x_i | Vector đặc trưng của mẫu hiện tại |
| x_zi | Vector đặc trưng của láng giềng được chọn |
...
| Công thức | Ý nghĩa |
|---|---|
| x_new_continuous = x_i + λ × (x_zi - x_i) | Nội suy tuyến tính giữa 2 mẫu |
...
#### b. CTGAN (Conditional Tabular GAN)
...
![Kết quả so sánh SMOTE-NC và CTGAN](ảnh_kết_quả_của_bạn)
*Hình X: So sánh hiệu suất mô hình giữa hai phương pháp cân bằng dữ liệu*
...
**SHAP** là một trong những công cụ quan trọng nhất trong Machine Learning — một phương pháp **model-agnostic** (không phụ thuộc vào loại mô hình) dùng để giải thích bất kỳ mô hình học máy nào.
...
Điều khiến Shapley values trở thành "đáp án đúng" là chúng thỏa mãn đồng thời **bốn thuộc tính then chốt (SHAP Axioms)**:

- **Efficiency (Hiệu suất):** Tổng các SHAP value bằng đúng chênh lệch giữa prediction của mô hình và base value. Không có phần đóng góp nào bị mất hoặc tạo thêm.
- **Symmetry (Đối xứng):** Nếu hai feature đóng góp như nhau trong mọi coalition, chúng nhận SHAP value bằng nhau.
- **Dummy (Tính vô hiệu):** Một feature không bao giờ làm thay đổi output sẽ có SHAP value bằng 0.
- **Additivity (Tính cộng gộp):** Với mô hình ensemble, SHAP value của mô hình tổng bằng tổng SHAP value của từng mô hình thành phần.
...
![Minh họa mô hình hộp đen](ảnh_hộp_đen_của_bạn)
*Hình X: Minh họa cho các mô hình "hộp đen"*

---

### 2. Tại sao cần XAI?

Hãy tưởng tượng chúng ta có một người yêu rất thông minh, tên là **AI** — luôn đúng và đoán được mọi thứ, nhưng lại mắc tính **"lầm lì"**.
...
|---|---|---|
| **Intrinsic** | "Người yêu thẳng thắn" | Mô hình có cấu trúc đơn giản, bản thân đã là một lời giải thích (Decision Tree, Linear Regression) |
| **Post-hoc** | "Người yêu bí ẩn" | Áp dụng cho mô hình hộp đen (Random Forest, Neural Network) — cần công cụ bên thứ ba để giải mã |
...
Thuật toán **SHAP** thuộc loại **Post-hoc**, hoạt động được ở cả phạm vi **Global lẫn Local**.

![Quy trình giải thích mô hình qua SHAP](ảnh_shap_pipeline_của_bạn)
*Hình X: Quy trình giải thích mô hình (XAI) thông qua SHAP*

Quy trình hoạt động của SHAP gồm 4 bước:

- 🔵 **Đầu vào (Models):** SHAP là phương pháp model-agnostic — dù là Logistic Regression đơn giản hay Random Forest phức tạp, SHAP đều có thể "thẩm vấn" để tìm ra câu trả lời.
...
- ⚙️ **Bộ giải thích SHAP (Explainer):** Dùng nền tảng từ **Game Theory** để phân bổ "phần thưởng" (kết quả dự đoán) cho từng "người chơi" (đặc trưng). Tổng đóng góp của các đặc trưng bằng chính xác mức chênh lệch giữa dự đoán hiện tại và dự đoán trung bình.
- 📈 **Kết quả giải thích:** Trình bày dưới dạng biểu đồ trực quan — **màu xanh** thể hiện yếu tố đang đẩy dự đoán về hướng "Stay", **màu đỏ** thể hiện yếu tố kéo về hướng ngược lại. Độ dài thanh thể hiện cường độ tác động.

> 💡 **Mục tiêu cuối cùng:** XAI không thay thế con người mà cung cấp **bằng chứng xác thực**. Nhà quản lý không chỉ biết AI dự đoán nhân viên ở lại, mà còn hiểu rõ *tại sao* — từ đó đưa ra quyết định hoặc điều chỉnh chính sách dựa trên sự hiểu biết thấu đáo.
...
3. **Hồi quy tuyến tính có trọng số:** Huấn luyện một mô hình hồi quy đơn giản trên các mẫu này — các mẫu có ít hoặc gần đủ số lượng đặc trưng được gán trọng số cao hơn (Shapley Kernel). Hệ số của mô hình hồi quy chính là Shapley value xấp xỉ.

![Cơ chế hoạt động của Kernel SHAP](ảnh_kernel_shap_của_bạn)
*Hình X: Cơ chế hoạt động của Kernel SHAP*
...
![Kết quả Logistic Regression](ảnh_kết_quả_lr_của_bạn)
*Hình X: Kết quả đánh giá mô hình Logistic Regression*
...
![SHAP Summary Plot - Logistic Regression](ảnh_shap_lr_của_bạn)
*Hình X: SHAP Summary Plot cho mô hình Logistic Regression*
...
![Kết quả Random Forest](ảnh_kết_quả_rf_của_bạn)
*Hình X: Kết quả đánh giá mô hình Random Forest*
...
- `shap.TreeExplainer(black_box_rf)`: Explainer chuyên dụng cho mô hình dạng cây — khai thác trực tiếp cấu trúc cây, không cần background dataset như Kernel SHAP. Độ phức tạp tính toán giảm từ **O(TL·2ᴹ)** (Kernel) xuống **O(TLD²)** (Tree), trong đó T là số cây, L là số lá, D là chiều sâu cây.
- `isinstance(shap_values_rf, list)`: Xử lý tương thích phiên bản — tùy phiên bản SHAP, kết quả trả về có thể là **list 2D** hoặc **mảng 3D**. Đoạn kiểm tra này đảm bảo code hoạt động đúng trên nhiều phiên bản thư viện.

![SHAP Summary Plot - Random Forest](ảnh_shap_rf_của_bạn)
*Hình X: SHAP Summary Plot cho mô hình Random Forest*